    ...
    <SafeAreaView style={{ flex: 1 }} edges={["top", "bottom"]}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          {current === "confirm" && <Step title="Seal Your Commitment" why="Review the promise you are choosing to practice with consistency, discipline, and presence."><Card style={styles.summary}><Summary label="BURNING DESIRE" value={burningDesire}/><Summary label="AFFIRMATION" value={affirmationCustom.trim() || (affirmationOn ? "Personalized daily affirmation" : "Off")}/><Summary label="SACRIFICE" value={sacrificeCat === "custom" ? sacrificeCustom : SACRIFICE_CATEGORIES.find((s) => s.key === sacrificeCat)?.label || "—"}/><Summary label="ALARMS" value={`${wakeEnabled ? formatTime(wakeTime) : "Wake off"} · ${sleepEnabled ? formatTime(sleepTime) : "Sleep off"}`}/><Summary label="REMINDERS" value={remindersEnabled ? `${reminderCount}x/day · ${reminderMode}` : "Off"}/></Card></Step>}
        </ScrollView>
